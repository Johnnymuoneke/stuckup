...
const PlannerScreen = () => {
    ...
    const pendingCount = items.filter((items) => items.purchased).length
    const highPriorityCount = items.filter((item) => !item.purchased && item.priority).length
    const totalQuantity = items.filter((item) => !item.purchased)
        .reduce((sum, item) => sum + item.quantity, 0)
    return (

        <ScrollView className="flex-1 bg-background py-7"
            ...
            <View className="gap-4 rounded-3xl border border-border bg-card/70 p-5">
                <View className="flex-row items-start justify-between">
                    <View className="flex-1 pr-4">
                        ...
                        <Text className="mt-1 text-3xl font-bold leading-9 text-foreground">Plan smarter, shop calmer.</Text>
                        <Text className="mt-2 text-sm leading-5 text-muted-foreground">Organize your next grocery run with categories, quantities, and priority in one place</Text>
                    </View>
                ...
                <View className="flex-row gap-2">
                    <View className="flex-1 rounded-2xl border border-border bg-background p-3">
                        <Text className="text-xs font-medium uppercase tracking-[1px] text-muted-foreground">pending</Text>
                    ...
                    <View className="flex-1 rounded-2xl border border-border bg-background/80 p-3">
                        <Text className="text-sx font-medium uppercase tracking-[1px] text-muted-foreground">Units</Text>
            ...
            <View className="px-1">
                <Text className="text-sm font-semibold text-muted-foreground">Build your list</Text>
