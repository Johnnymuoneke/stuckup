...
const InsightsPrioritySection = () => {
    ...
    const highPriorityTone = highPriority === 0 ? "Everthing critical is covered" : "Handle these first for a smoother trip";
    ...
    return (
        <View className="rounded-3xl border border-border bg-card p-4">
            <View className="flex-row items-center justify-between">
                ...
                <View
                className={`rounded-full px-3 py-1  ${highPriority ? "bg-priority-high" :"text-priority-low"}`}
                >
                    <Text className={`text-xs font-bold  ${
                        highPriority ? "text-priority-high-foreground" : "text-priority-low-foreground"
                    }`}>{highPriority ? "ACTION" : "CLEAR"}</Text>
                </View>
            </View>
